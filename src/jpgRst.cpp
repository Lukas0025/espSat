...
namespace JpgRst {
    ...
    static IRAM_ATTR void convert_line_format(uint8_t * src, uint8_t * dst, size_t width, size_t line) {
        int i=0, o=0, l = width * 2;
    ...
    bool convert_image(uint8_t *src, uint16_t width, uint16_t height, uint8_t quality, jpge::output_stream *dst_stream) {
        int num_channels = 3;
        ...
        for (int i = 0; i < height; i++) {
            convert_line_format(src, line, width, i);
    ...
    buffer_t rgb5652jpg(uint8_t *src, size_t src_len, uint16_t width, uint16_t height, uint8_t quality) {
        buffer_t out;
        ...
        if(!convert_image(src, width, height, quality, &dst_stream)) {
            free(jpg_buf);
            return out;
        }

        out.data = jpg_buf;
        out.len  = dst_stream.get_size();
    ...
    #define DCT1D(s0, s1, s2, s3, s4, s5, s6, s7) \
        ...
        bool jpeg_encoder::jpg_open(int p_x_res, int p_y_res, int src_channels)
        {
            ...
            /*if (m_withRst) {
                emit_marker(M_DRI);
                uint32_t drinum = m_mcus_per_row;
                emit_word(uint16(drinum >> 16));
                emit_word(uint16(drinum & 0xFFFF));
            }*/
            emit_sos();
